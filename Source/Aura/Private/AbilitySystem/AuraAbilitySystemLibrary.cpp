...
{
	UCharacterClassInfo* CharacterClassInfo = GetCharacterClassInfo(WorldContextObject);
	if (CharacterClassInfo == nullptr) return;//클라이언트라면 바로 return
	...
	//SecondaryAttributes는 infinite->따로 infinite용 하나 더 둠
	FGameplayEffectContextHandle SecondaryAttributesContextHandle = ASC->MakeEffectContext();
...
void UAuraAbilitySystemLibrary::GiveStartupAbilities(const UObject* WorldContextObject, UAbilitySystemComponent* ASC, ECharacterClass CharacterClass)
{
	...
	{
		FGameplayAbilitySpec AbilitySpec = FGameplayAbilitySpec(AbilityClass, 1);//CommonAbilities에는 HitReact나 Death같은 어빌리티만 있어 레벨 1로만 설정해도 된다.
	...
	ECharacterClass CharacterClass, int32 CharacterLevel)
{
	...
	return static_cast<int32>(XPReward); //만약 게임의 XP가 float인 점이 중요하다면 함수를 float반환형으로 바꾸도록 
}
...
{
	FCollisionQueryParams SphereParams; //구조체 찾아보면 다 디폴트 값이 있음, 필요한 것만 수정할 것임.
	SphereParams.AddIgnoredActors(ActorsToIgnore);
	
	TArray<FOverlapResult> Overlaps;
	if (UWorld* World = GEngine->GetWorldFromContextObject(WorldContextObject, EGetWorldErrorMode::LogAndReturnNull))//World가져오는데 실패 했다면 EGetWorldErrorMode로 뭐 할지 결정할 수 있다.
	{
		World->OverlapMultiByObjectType(Overlaps, SphereOrigin, FQuat::Identity, FCollisionObjectQueryParams(FCollisionObjectQueryParams::InitType::AllDynamicObjects), FCollisionShape::MakeSphere(Radius), SphereParams);
		for (FOverlapResult& Overlap : Overlaps)
		{
			//Overlap.GetActor()->Implements<UCombatInterface>(); //interface템플릿함수. Interface를 구현했으면 참 반환
...
void UAuraAbilitySystemLibrary::GetClosestTargets(int32 MaxTargets, const TArray<AActor*>& Actors, TArray<AActor*>& OutClosestTargets, const FVector& Origin)
{
	...
	while (NumTargetsFound < MaxTargets) //가까운 순으로 잡아 넣기 O(N*M)
	{
...
FGameplayEffectContextHandle UAuraAbilitySystemLibrary::ApplyDamageEffect(const FDamageEffectParams& DamageEffectParams) //AAuraProjectile::OnSphereOverlap에서 사용
{
	const FAuraGameplayTags& GameplayTags = FAuraGameplayTags::Get();
	...
	//SpecHandle에 SetByCaller값 넣어주기
	UAbilitySystemBlueprintLibrary::AssignTagSetByCallerMagnitude(SpecHandle, DamageEffectParams.DamageType, DamageEffectParams.BaseDamage);
	UAbilitySystemBlueprintLibrary::AssignTagSetByCallerMagnitude(SpecHandle, GameplayTags.Debuff_Chance, DamageEffectParams.DebuffChance);//chance값 만들기
	UAbilitySystemBlueprintLibrary::AssignTagSetByCallerMagnitude(SpecHandle, GameplayTags.Debuff_Damage, DamageEffectParams.DebuffDamage);
	UAbilitySystemBlueprintLibrary::AssignTagSetByCallerMagnitude(SpecHandle, GameplayTags.Debuff_Duration, DamageEffectParams.DebuffDuration);
	UAbilitySystemBlueprintLibrary::AssignTagSetByCallerMagnitude(SpecHandle, GameplayTags.Debuff_Frequency, DamageEffectParams.DebuffFrequency);

	DamageEffectParams.TargetAbilitySystemComponent->ApplyGameplayEffectSpecToSelf(*SpecHandle.Data);//*SpecHandle.Data 이렇게만 해도 값을 낸다.
