...
TArray<USceneComponent*> APointCollection::GetGroundPoints(const FVector& GroundLocation, int32 NumPoints, float YawOverride)
{
	...
	for (USceneComponent* Pt : ImmutablePts)
	{
		...
		if (Pt != Pt_0) //Pt_0은 루트로 사용되는 Pt이므로
		{
			FVector ToPoint = Pt->GetComponentLocation() - Pt_0->GetComponentLocation();
			ToPoint = ToPoint.RotateAngleAxis(YawOverride, FVector::UpVector); //Pt_0중심으로 회전(쏠 때마다 방향 다르게 하기 위해)
		...
		UAuraAbilitySystemLibrary::GetLivePlayerWithinRadius(this, IgnoreActors, TArray<AActor*>(), 1500.f,GetActorLocation());//GetLivePlayerWithinRadius: 범위 내 라이브 플레이어 다 모으는 함수->다 인터페이스 구현한 것들일 것이므로 ActorsToIgnore은 다 무시하고 IgnoreActors에 넣어서 다른 것들에 무시되도록 한다.
		...
		Pt->SetWorldRotation(UKismetMathLibrary::MakeRotFromZ(HitResult.ImpactNormal));//ImpactNormal: 맞은 곳에서 수직으로
